...
| 结果 | 完成判定 | 当前进展 |
|---|---|---|
| 入口收束 | systemd 不再依赖 legacy shell wrapper，主链路使用 `run_daily.py` / `run_daily_batch.py` | ✅ systemd 已切换至 `run_daily_batch.py --stage candidate`（PR #125）。旧 legacy 入口从仓库移除（git 历史可追溯）。|
