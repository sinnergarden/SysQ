...
Transform 可用于表达信号或特征后处理链，例如 winsorize、neutralize、daily_zscore。行业、市值等中性化依赖的数据应作为显式输入。

超参搜索属于 Signal Research 的 matrix experiment，可通过多个 generator config 或参数网格展开，不单独改变 Use Case。例如：
...
qsys/research/signal_pipeline.py
...
qsys/research/signal_analytics.py
SignalAnalytics
...
qsys/backtest/signal_backtest_pipeline.py
SignalBacktestPipeline
...
risk:
  ...
  max_total_turnover: 0.2
  max_position_weight: 0.08
  require_operator_confirm: true
  reject_if_suspended: true
  reject_if_price_limit: true
...
* 需要 daily run manifest 与 research/backtest/candidate 关联。
...
BacktestedStrategy → Candidate → Shadow → Production → Deprecated
...
data/research/promotions/production.yaml
...
* Daily Trading 尚未接入 promotion pointer。
...
UC-1  同步并校验所需数据
UC-2  将新 feature 加入 feature registry 和指定 feature list
UC-4  使用新 feature list 运行 signal research
UC-5  对比新旧信号表现
UC-7  对候选信号运行策略回测
UC-9  根据 evidence 决定是否晋级
...
1. 实现新的 generator，并满足 RollingSignalGenerator 协议
2. 在 UC-4 research config 中新增 generators[].type
## 3. 复用既有 labels、feature_list、transforms、calendar
4. 通过 SignalResearchPipeline 生成 SignalRun
## 5. 通过 SignalAnalytics 和 SignalBacktestPipeline 评估
...
1. 实现新的 strategy target builder
2. 在 UC-7 backtest config 中新增 strategy.type
## 3. 使用已有 SignalRunRef 运行 SignalBacktestPipeline
4. 对结果进行 benchmark-aware 评估
## 5. 根据 evidence 进入 Candidate Promotion
...
对比实盘或 shadow daily run 与对应 backtest 的偏差，评估模拟到实盘的一致性。

**输入**

reconciliation_id: rec_lgbm_top20_202606
backtest_ref:
  strategy_run_id: top20_weekly
  backtest_id: bt_lgbm_csi800_10d_top20_2024_2025
daily_runs:
