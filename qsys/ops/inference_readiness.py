"""Reusable fail-closed readiness checks for daily inference inputs."""
...
def check_inference_ready(
    ...
    *,
    ...
    signal_request = str(trade_date or "").strip()
    signal_date = _normalise_date(signal_request)
    signal_request_ok = signal_request.lower() == "auto" or signal_date is not None
    results.append(
        (
            "signal_date request",
            signal_request_ok,
            (
                ...
                if signal_request.lower() == "auto"
                else f"valid: {signal_date}"
                if signal_date
                else f"invalid: {trade_date}"
            ),
        )
    )
    results.append(("strategy_id non-empty", bool(strategy_id), strategy_id or "empty"))
    if not signal_request_ok or not strategy_id:
        return results
    ...
    try:
        config = (
            ...
            else load_strategy_config(strategy_id, project_root)
        )
