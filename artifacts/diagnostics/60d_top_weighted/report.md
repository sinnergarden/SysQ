...
| 验收条件 | 结果 | 详情 |
|:---|---|:---|
| ① Top20/Top50 mean_ret 提升 | ❌ | 0.1018 → ~0.1024，持平 |
| ② Top20/Top50 hit_rate 不下降 | ❌ | 54.18% → ~53.07%，下降 |
| ③ TopK inner RankIC 提升 | ✅ | −0.0022 → 0.0618 |
| ④ Overall IC/RankIC 不明显劣化 | ❌ | IC 0.0713 → 0.0644，RankIC 0.0651 → 0.0422 |
| ⑤ top-bottom spread 扩大 | ✅ | 0.036 → 0.041 小幅改善 |

### 核心数据

| Metric | baseline_no_weight | Best weighted | 结论 |
|:---|---:|:---:|:---:|
| Overall IC | 0.0713 | ~0.0644 | Worse |
| RankIC | 0.0651 | ~0.0422 | Worse |
| ICIR | 0.818 | ~0.686 | Worse |
| Top20 mean_ret | 0.1018 | ~0.1024 | Flat |
| Top20 hit_rate | 54.18% | ~53.07% | Worse |
| Top50 mean_ret | 0.0798 | ~0.0814 | Flat |
| Top100 mean_ret | 0.0679 | ~0.0709 | Slightly better |
| Top20 inner RankIC | −0.0022 | 0.0618 | Better |
| top10-bot20 spread | 0.0360 | ~0.0405 | Slightly better |

### 总体判断

**实验不通过，不纳入 60d baseline / candidate baseline。** 不推荐进入 LambdaRank/pairwise ranking。

加权方案的 TopK inner RankIC 确实改善了（从接近 0 到 0.06），说明 head-weighted 训练在排序顶部做了更精细的区分。但每项头部加权尝试都导致了 Overall IC 和 RankIC 的下降，Top20 hit_rate 也在变差。Inner RankIC 的改善不足以弥补整体信号质量的下降。

## 11. Reproduce

```bash
# 训练 4 个 weighting scheme（约 60 分钟，依赖 per-window feature cache）
python scripts/dev/run_60d_top_weighted_lgbm.py

# 评估（约 2 分钟）
python scripts/dev/evaluate_60d_top_weighted.py
```

- 依赖已有 per-window feature cache（由 60d superset write-through 生成）
- 不重算 feature
- prediction parquet 可由脚本复现，但不提交 git（`artifacts/diagnostics/60d_top_weighted/predictions/*.parquet`）
- 同一 scheme 重跑覆盖同名输出
