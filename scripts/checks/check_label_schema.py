#!/usr/bin/env python3
"""Validate label parquet/csv schema.

Required columns: trade_date, instrument, label_id, horizon, label_value.
...
REQUIRED_COLUMNS = {"trade_date", "instrument", "label_id", "horizon", "label_value"}
OPTIONAL_COLUMNS = {
...
def check_label_schema(path: Path) -> dict:
    if path.is_dir():
        ...
        if loaded is None:
            if suffix == ".parquet":
                ...
                result["errors"].append(
                    f"{f}: parquet dependency not available"
                )
            else:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Validate label parquet/csv schema"
    )
    ...
    result = check_label_schema(path)
    print(json.dumps(result, indent=2))
