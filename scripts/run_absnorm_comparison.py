...
import warnings
warnings.warn(
    "DEPRECATED: run_absnorm_comparison.py is superseded by UC-standard entrypoints. Scheduled for removal.",
...
def _collect_artifact_metrics(experiment_dir: Path) -> dict[str, Any]:
    root = _resolve_artifact_root(experiment_dir)
    metrics = _read_json(root / "metrics.json")
    signal_metrics = _read_json(root / "signal_metrics.json")
    execution_summary = _read_json(root / "execution_summary.json")
    exposure_summary = _read_json(root / "exposure_summary.json")
    group_returns = _read_csv(root / "group_returns.csv")
...
def _resolve_artifact_root(experiment_dir: Path) -> Path:
    candidates = [experiment_dir / "data" / "experiments", experiment_dir]
    for candidate in candidates:
        if (candidate / "metrics.json").exists() or (candidate / "signal_metrics.json").exists():
            return candidate
    return candidates[0]
...
def _build_markdown(summary: pd.DataFrame, *, start: str, end: str, universe: str, top_k: int, audit_path: Path) -> str:
    ...
    lines += ["## Comparison table", "", _frame_to_markdown(compact), ""]
