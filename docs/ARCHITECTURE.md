...
| 维度 | 目标态 | 当前现实 | 差距 |
|------|--------|---------|------|
| 入口 | `run_daily.py` + `run_daily_batch.py` | systemd 已切换至 `run_daily_batch.py --stage candidate`（PR #123）。旧 legacy 入口不再被 systemd 调用。| ✅ systemd cutover 完成 |
...
| Ops SOP | 反映当前入口和流程 | SOP 已更新至当前 systemd 入口 | ✅ 已同步 |
