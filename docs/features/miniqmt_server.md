...
- A 股最小交易单位校验
- BUY 订单可用资金检查（基于 `limit_price` 或本地缓存价格估算）
- SELL 订单 `available_volume` 检查
- 批量请求内的顺序化占资校验，避免同批 BUY 合计超出可用资金
...
- 对已成功落单的 `request_id` 做持久化幂等保护
- 相同 payload 重试时回放原始提交结果，而不是重复生成新订单
- 不同 payload 复用同一个 `request_id` 时明确返回冲突错误，避免误下重复单
