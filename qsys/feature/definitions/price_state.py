...
def build_daily_price_state_features(df: pd.DataFrame) -> pd.DataFrame:
    out = df.copy()
    prev_close = out.groupby("ts_code", sort=False)["close"].shift(1)
    intraday_range = (out["high"] - out["low"]).replace(0, np.nan)

    out["close_to_open_gap_1d"] = out["open"] / prev_close - 1
    out["open_to_close_ret"] = out["close"] / out["open"].replace(0, np.nan) - 1
    ...
    out["intraday_reversal_strength"] = -(out["close"] / prev_close - 1) * out["open_to_close_ret"]
    return out
