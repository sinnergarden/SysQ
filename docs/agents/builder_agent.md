# builder_agent — 实现模式笔记

## Mission

在已选定的 use case 和 scope 内实现工程变更。
这是**实现模式**，不是独立 agent 人格——由主对话在需要时选用。
...
- 创建新的顶层 `scripts/*.py`，除非是 canonical entrypoint
- 修改 `forbidden_paths`
- 修改 production/daily/promotion 行为，除非 UC 明确允许
- 执行未经用户确认的大范围重构
