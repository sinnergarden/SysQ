...
### Ablation 年份表（2026-06-17 重跑 — 修正 announcement-level qoq）

> ⚠️ 2026-06-16 版本的 shareholder/full 数据有bug（日频 shift 代替公告级 qoq，变化值 ~0）。
> 以下为修复 `load_shareholder_data` + `build_shareholder_features` 后的正确结果。

| Year | v2 baseline | +margin | +shareholder | +full |
|------|:--------:|:------:|:----------:|:----:|
| 2020 | 0.3203 | 0.3629 | **0.4354** | **0.4642** |
| 2021 | 0.2541 | 0.2406 | **0.3322** | **0.4138** |
| 2022 | 0.3276 | 0.3343 | **0.3845** | **0.4174** |
| 2023 | 0.5219 | 0.5459 | **0.5779** | **0.5681** |
| 2024 | 0.4591 | 0.4728 | **0.4899** | **0.5099** |
| 2025 | 0.4502 | 0.5011 | **0.5320** | **0.5575** |
| **Overall** | **0.3980** | **0.4158** | **0.4682** | **0.4991** |
| **ICIR** | 2.971 | 2.908 | **3.797** | **4.851** |

**结论（修正后）：**
- Margin (+0.018 IC, +0.7% ICIR, 4/6 年正) — Weak Pass
- Shareholder **(+0.070 IC, +27.8% ICIR, 6/6 年全正 → Strong Pass ✅)** — 公告级 qoq 计算修复后筹码集中度信号显著
- Full **(+0.101 IC, +63.3% ICIR, 6/6 年全正 → Strong Pass ✅)** — margin 和 shareholder 特征正交性好，叠加后 IC 和 ICIR 全面提升
- 两个特征集都通过 Strong Pass 标准（IC > v2+0.02, ICIR 提升, 多数年份改善）

## 信号路径规则

signal_id: `fwd_ret_180d_raw__daily_zscore`
run_id: `rolling__<experiment_id>__<generator_id>__fwd_ret_180d_raw__daily_zscore__<start>_<end>`
