...
- pre_open trains a maturity-safe model on the trailing window ending at signal_date
- pre_open generates signal basket and plans
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--retrain_cadence", choices=["daily", "weekly"], default="daily", help="Retrain every day or once per week using the last trained model in between")
...
def run_pre_open(args: argparse.Namespace, paths: BackfillPaths, signal_date: str, execution_date: str, log_path: Path, *, train_in_preopen: bool) -> dict[str, object]:
    cmd = [
        sys.executable,
    ...
    summary_path = (PROJECT_ROOT / args.model_path / "training_summary.csv").resolve()
    if not summary_path.exists():
        raise FileNotFoundError(f"Training summary not found after preopen training: {summary_path}")
    summary = pd.read_csv(summary_path).iloc[-1].to_dict()
    return {
        "infer_date": summary.get("infer_date"),
        "last_train_sample_date": summary.get("last_train_sample_date"),
        "max_label_date_used": summary.get("max_label_date_used"),
        "is_label_mature_at_infer_time": bool(summary.get("is_label_mature_at_infer_time")),
        "train_start": summary.get("train_start"),
        "train_end": summary.get("train_end"),
        "train_end_requested": summary.get("train_end_requested"),
...
def main() -> None:
    ...
    for idx, execution_date in enumerate(execution_days, start=1):
        ...
        try:
            should_train = args.retrain_cadence == "daily" or not trained_signal_dates or pd.Timestamp(signal_date).weekday() == 0
            preopen_audit = run_pre_open(args, paths, signal_date, execution_date, day_log, train_in_preopen=should_train)
            if should_train:
                trained_signal_dates.append(signal_date)
            append_progress(paths, "pre_open_done", execution_date=execution_date, signal_date=signal_date, retrain_cadence=args.retrain_cadence, trained_in_preopen=should_train, **preopen_audit)
